...
const ServiceForm = () => {
    ...
    const [serviceData, setServiceData] = useState({
        ...
        is_active: 1,
        has_variation: false,
        detail: '', // Make sure to include 'detail' field
    });

    // Include IDs in variations
    const [variations, setVariations] = useState([{ id: null, name: '', price: '' }]); // New state for variations with IDs
    const [deletedVariationIds, setDeletedVariationIds] = useState([]); // New state to track deleted variations
    const [adding, setAdding] = useState(false);
    const [categories, setCategories] = useState([]);
    const [labels, setLabels] = useState([]);
    const [tags, setTags] = useState([]);
    ...
    const fetchServiceDetail = async (serviceId) => {
        ...
        try {
            ...
            if (data.variation && data.variation.length > 0) {
                ...
                setVariations([{ id: null, name: '', price: '' }]);
            }
        } catch (error) {
            console.error("Error fetching service detail", error);
            if (error.response && error.response.status === 401) {
    ...
    const addNewVariation = () => {
        setVariations([...variations, { id: null, name: '', price: '' }]);
    };
                        ...
                        {serviceData.has_variation && (
                            <div className="mb-4 sm:mb-6">
                                ...
                                {variations.map((variation, index) => (
                                    <div key={index} className="grid grid-cols-2 gap-4 sm:gap-6 mb-4">
                                        <div>
                                        ...
                                        <div className="flex items-center">
                                            <button
                                                type="button"
                                                className="bg-red-500 text-sm sm:text-base font-semibold text-white px-3 py-1 rounded"
